...
def GenYMLNAS_cyl_SA(bdffilename):
    ...
    ##### read paramters
    paras_info = bdf_name + "paras.txt"

    with open(paras_info) as file2:
        lines2 = file2.readlines()

    all_paras = []
    for j in range(len(lines2)):
        all_paras.append(lines2[j].split('\n')[0].replace(' ', '').split(','))

    paras_dict = {}

    for j in range(len(all_paras[0])):
        paras_dict[all_paras[0][j]] = all_paras[-1][j]
    ...
    with open(newSF, 'w') as fnew:
        ...
        fnew.write('  rel_tol: 1.0e-6\n')
        fnew.write('  abs_tol: 1.0e-9\n')
        fnew.write('\n')
        fnew.write('\n')
        fnew.write('structure:\n')
        fnew.write('  name: "cylinder"\n')
        fnew.write('  parameters:\n')
        for h, k in paras_dict.items():
            fnew.write('    ' + h + ': ' + str(k) + '\n')
        # for i, j in IV.items():
        #     fnew.write('    ' + i + ': ' + str(j) + '\n')
        fnew.write('    _phi10: 0\n')
        ...
        #     else:
        #         countaf += 1
        phi_vals = str(['_phi10', '_phi11', '_phi12', '_phi13', '_phi14'])
        fnew.write('    - name: a2\n')
        fnew.write('      function: f1\n')
        fnew.write('      coefficients:\n')
        fnew.write('        theta: [0, 45, 90, 135, 180]\n')
        fnew.write('        phi: '+phi_vals+'\n')
        fnew.write('        m: 1\n')
        fnew.write('    - name: a3\n')
        fnew.write('      function: f1\n')
        fnew.write('      coefficients:\n')
        ...
        #         fnew.write(
        #             '    coefficients: ["' + list(var_name.keys())[0] + '", "' + list(var_name.keys())[1] + '"]\n')
        fnew.write('  - name: "f1"\n')
        fnew.write("    type: 'script'\n")
        fnew.write("    file_name: 'users_function'\n")
        fnew.write("    function_name: 'svFiberAngle'\n")
        ...
        fnew.write('    - name: "buckling analysis"\n')
        ...
        fnew.write('            "' + pargs[1] + '"\n')
        fnew.write('          ]\n')
        fnew.write('      step_result_file: "' + pargs[1] + '"\n')
        fnew.write('\n')
        fnew.write('\n')
        fnew.write('study:\n')
        fnew.write('  method:\n')
        fnew.write('    multidim_parameter_study:\n')
        fnew.write('      partitions: [3, 3, 3, 3, 3]\n')
        lyr_lud_remove_dupli = []
        for h in lyr_lud.values():
            lyr_lud_remove_dupli.append(h[0])

        lyr_lud_remove_dupli = list(set(lyr_lud_remove_dupli))
        ...
        fnew.write('      - name: "_phi10"\n')
        fnew.write('        type: "continuous"\n')
        fnew.write('        bounds: [-90, 90]\n')
        fnew.write('      - name: "_phi11"\n')
        fnew.write('        type: "continuous"\n')
        fnew.write('        bounds: [-90, 90]\n')
        fnew.write('      - name: "_phi12"\n')
        fnew.write('        type: "continuous"\n')
        fnew.write('        bounds: [-90, 90]\n')
        fnew.write('      - name: "_phi13"\n')
        fnew.write('        type: "continuous"\n')
        fnew.write('        bounds: [-90, 90]\n')
        fnew.write('      - name: "_phi14"\n')
        fnew.write('        type: "continuous"\n')
        fnew.write('        bounds: [-90, 90]\n')
...
def GenYMLABQ_cyl_SA(inpfilename):
    # inpfilename = 'tf9600-2000-120x80.inp'
    ...
    for i in range(len(layup)):
        layup_info[lines2[4 * i + 1].split('\n')[0]] = {'type': lines2[4 * i + 2].split('\n')[0],
                                                        'ply_thickness': lines2[4 * i + 3].split('\n')[0],
                                                        'orientation': lines2[4 * i + 4].split('\n')[0].replace('[',
                                                                                                                '').replace(
                                                            ']', '').replace("'", '')}
    ...
    for i in range(int(NumIV / 3)):
        ...
        IV_name[iv_list[3 * i].strip() + iv_list[3 * i + 1].strip()] = {'var name': iv_list[3 * i + 1].strip(),
                                                                        'var value': iv_list[3 * i + 2].strip()}
    ...
    if os.path.getsize(scriptinfo) == 0:
        ...
        for i, j in dict2.items():
            script_paras_info[i]= [j[0], j[1], j[2:-1], j[-1]]
    ...
    with open(newSF, 'w') as fnew:
        ...
        fnew.write('    R: 12\n')
        fnew.write('    k11: 0.0833\n')
        fnew.write('    k22: 0\n')
        for i in sorted(list(set(IV_name_removeDuplicate))):
            fnew.write('    ' + i + ': ' + '0' + '\n')
        ...
        for i in range(len(id_layup_expressin)):
            ...
            fnew.write('      coefficients:\n')
            if layup_info[layup[id_layup_expressin[i]]]['orientation'] != 'scripts':
                for j, k in IV_name.items():
                    if layup[id_layup_expressin[i]] in j:
                        fnew.write('        ' + k['var name'] + ': ' + j + '\n')
            else:
                for h in range(len(script_paras_info[layup[id_layup_expressin[i]]][-2])):
                    fnew.write('        ' + script_paras_info[layup[id_layup_expressin[i]]][-2][h] + '\n')
            fnew.write('        m: '+script_paras_info[layup[id_layup_expressin[i]]][-1]+'\n')
        fnew.write('  design:\n')
        fnew.write('    file: "'+inp_name+'"\n')
        ...
        for i in range(len(id_layup_expressin)):
            ...
            if layup_info[layup[id_layup_expressin[i]]]['orientation'] == 'scripts':
                fnew.write('    type: "script"\n')
                fnew.write('    file_name: "' + script_paras_info[layup[id_layup_expressin[i]]][0] + '"\n')
                fnew.write('    function_name: "' + script_paras_info[layup[id_layup_expressin[i]]][1] + '"\n')
            else:
                fnew.write('    type: "expression"\n')
        ...
        result = {}
        for key, value in IV_name.items():
            if value not in result.values():
                result[key] = value
        ps_var_iv = []
        for i in sorted(list(set(IV_name_removeDuplicate))):
            for h, k in result.items():
                if k['var name'] == i:
                    ps_var_iv.append(float(k['var value']))
        ...
        for i in sorted(list(set(IV_name_removeDuplicate))):
            fnew.write('      - name: "' + i + '"\n')
            fnew.write('        type: "continuous"\n')
            fnew.write('        bounds: [' + str(-90) + ', ' + str(90) + ']\n')
        fnew.write('  responses:\n')
        fnew.write('    data_form: "explicit"\n')
...
def GenYMLABQ_cyl_PS(inpfilename):
    # inpfilename = 'tf9600-2000-120x80.inp'
...
def GenYMLABQ_cyl_PS_lhs(inpfilename):
    # inpfilename = 'tf9600-2000-120x80.inp'
...
def GenYMLABQ_cyl_OPTIM(inpfilename):
    # inpfilename = 'tf9600-2000-120x80.inp'
