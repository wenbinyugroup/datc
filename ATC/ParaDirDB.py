...
class ParaDirDB(AFXDataDialog):
    ...
    def __init__(self, form):
        ...
        FXMAPFUNC(self, SEL_COMMAND, self.ID_MAT, ParaDirDB.onCmdMAT)
        FXMAPFUNC(self, SEL_COMMAND, self.ID_INCLUSION_MAT, ParaDirDB.onCmdFiberMAT)
        FXMAPFUNC(self, SEL_COMMAND, self.ID_FIBER, ParaDirDB.onCmdMATRIX)
        ...
        tablecoeff = AFXTable(vf, 12, 6, 3, 6, form.tablecoeffKw, 5, AFXTABLE_EDITABLE | LAYOUT_FILL_X)
        ...
        tablecoeff.setLeadingRowLabels('Layer\tVariables\tLower Bound\tUpper Bound\tPartitions')
        ...
        vf = FXVerticalFrame(GroupBox_3, FRAME_SUNKEN | FRAME_THICK | LAYOUT_FILL_X,
                             0, 0, 0, 0, 0, 0, 0, 0)
        vf.setSelector(99)
        table_rad = AFXTable(vf, 3, 4, 100, 4, form.fiber_radius_tableKw, 0, AFXTABLE_EDITABLE | LAYOUT_FILL_X)
        ...
        table_rad.setLeadingRowLabels('Lower bound\tUpper bound\tPartitions')
        ...
        #       button is checked in the RSG Dialog Builder dialog.
        vf.setSelector(99)
        table_plies = AFXTable(vf, 12, 6, 100, 6, form.pliesKw, 0, AFXTABLE_EDITABLE | LAYOUT_FILL_X)
        ...
        table_plies.setLeadingRowLabels('Layer\tVariables\tLower bound\tUpper bound\tPartitions')
    ...
    def onCmdDone(self, sender, sel, ptr):
        ...
        for i in range(1, self.tablecoeff.getNumRows()):
            ...
            globalVar.add_to_advP(self.tablecoeff.getItemText(i, 4))
...
# #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        ...
        for i in cm[mat].elastic.table:
            for j in range(0, len(i)):
                t.append(i[j])
        # f = open(jobname + "LaminaProp.txt", "w")
        # f.write(str(t) + '\n')
        # f.close()
        
        
        # f = open(jobname + "layer-info.txt", "w")
        # eq_list = []
        # layer_list = []
        # type_list = []
        # for i in layup:
            # if i in globalVar.Layer().get_defineFiberAngles().keys():
                # eq_list.append(globalVar.Layer().get_defineFiberAngles()[i]["orientation"][0])  # ['eq1', 'eq1']
                # layer_list.append(i)
                # type_list.append(globalVar.Layer().get_defineFiberAngles()[i]["type"])
        # f.write(json.dumps(globalVar.Layer().get_defineFiberAngles(), indent=10))
        # f.write(str(layup) + '\n')
        # f.write(str(eq_list) + '\n')
        ...
        for i in range(1, self.table_plies.getNumRows()):
            if self.table_plies.getItemText(i, 1) != '':
                var_dump_plies.append([self.table_plies.getItemText(i, 1),
                                                          ...
                                                          self.table_plies.getItemText(i, 3),
                                                          self.table_plies.getItemText(i, 4)])
        
        partitions = []
        for i in range(1, self.tablecoeff.getNumRows()):
            if self.tablecoeff.getItemText(i, 5) != '':
                partitions.append(int(self.tablecoeff.getItemText(i, 5)))
        for i in range(1, self.table_plies.getNumRows()):
            if self.table_plies.getItemText(i, 5) != '':
                partitions.append(int(self.table_plies.getItemText(i, 5)))
        ...
        if 2 not in choice:
            os.system('python ' + currentWD + '\\PyYML\\GenerateYML.py abaqus_ps ' +jobname)
        else:
            os.system('python ' + currentWD + '\\PyYML\\GenerateYML-cyl.py abaqus_ps ' +jobname)
        
        os.system('datc vs_design_ps.yml')
